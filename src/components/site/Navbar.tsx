...
function Chevron() {
  return (
    <span
      aria-hidden
      className="block h-[6px] w-[6px] -translate-y-[2px] rotate-45 border-r border-b border-muted"
    />
  );
}
...
export default function Navbar({
  ...
  return (
    <header
      className="sticky top-0 z-50 border-b border-rule bg-paper/[0.94] backdrop-blur-[14px]"
      onMouseLeave={() => setOpenMenu(null)}
    >
      {/* ── Desktop bar ───────────────────────────────────────────────── */}
      <div className="shell hidden h-[86px] grid-cols-[1fr_auto_1fr] items-center gap-6 min-[1400px]:grid">
        <nav className="flex items-center gap-7 min-[1560px]:gap-[38px]">
          <div
        ...
        <nav className="flex items-center justify-end gap-[22px] min-[1560px]:gap-[26px]">
          <LanguageSwitcher />
          <Link
            onClick={closeAll}
          ...
          <Link
            ...
            className="nav-link border border-ink/38 px-[22px] py-[11px] transition-colors hover:border-accent hover:bg-accent hover:text-deep"
            onMouseEnter={() => setOpenMenu(null)}
          >
            {t("preOrder")}
          </Link>
      ...
      <div className="min-[1400px]:hidden">
        ...
        {drawerOpen && (
          <div className="flex flex-col border-t border-rule bg-card px-6 pt-[14px] pb-[30px]">
            {mobileLinks.map((link) => (
              <Link
                onClick={closeAll}
                key={link.href}
                href={link.href}
                className="border-b border-rule py-[15px] text-[19px] font-extralight text-ink"
              >
      ...
      {openMenu && (
        <div className="hidden border-t border-rule bg-card min-[1400px]:block">
            ...
            <div className="flex flex-col gap-[2px]">
              {menus[openMenu].items.map((item) => (
                <Link
                  ...
                  key={item.href}
                  href={item.href}
                  className="flex min-w-[420px] items-baseline justify-between gap-10 border-b border-rule py-[9px] text-[26px] font-extralight text-ink transition-colors hover:text-accent"
                >
